...
const CardTypes = [ 'BoxCover' ];
const BindingSuffixes = [''];

const PortraitTypeList = [ 'Portrait-Front', 'PortraitBottom-Front' ];
...
function createInterface( diy, editor ) {
	...
	var BottomPortraitTabArray = layoutPortraitsWithPanels( diy, bindings, 'PortraitBottom', null, true, false, false );
	var BottomPortraitTab = BottomPortraitTabArray[0];
	BottomPortraitTabArray.splice( 0, 1 );
	...
	BottomPortraitTab.addToEditor(editor, 'Bottom Portrait');
...
function paintFront(g, diy, sheet) {
    ...
    PortraitList[1].paint( g, sheet.getRenderTarget() );

    var hsb = $$tint.tint;
    var tintable_image = ImageUtils.get('ArkhamHorrorLCG/templates/AHLCG-BoxCoverTintable.png');
	let tinted_image = ImageUtils.tint(tintable_image, hsb);
    sheet.paintImage(g, tinted_image, diy.settings.getRegion('AHLCG-BoxCover-Tint-region'));
